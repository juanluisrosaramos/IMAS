...
public class GameSettings implements java.io.Serializable {    
    ...
    public ArrayList<GarbageInfo> detectBuildingsWithGarbage(int row, int col) {
        //context: called by an scout passing near a building with garbage
        //given a x,y it returns an array of buildings containing garbage 
        ArrayList<GarbageInfo> result = new ArrayList<>();
        int NROWS =  this.getMap().length; 
        int NCOLS = this.getMap()[0].length;
        ...
        for(int i=-1;i<2;i++){
            ...
            if ((r_>=0) && (r_<NROWS)){
                for(int j=-1;j<2;j++){                
                    int c_ =col +j;
                    if((c_>=0) && (c_<NCOLS)){
                        if(map[r_][c_] instanceof SettableBuildingCell){
                            Map<GarbageType, Integer> garbage = ((SettableBuildingCell)map[r_][c_]).detectGarbage();
                            if(!garbage.isEmpty()){
                                SettableBuildingCell building = (SettableBuildingCell)map[r_][c_];
                                GarbageInfo entry = new GarbageInfo(building, garbage);
                                result.add(entry);
                            }
                        }
                    }
    ...
    public ArrayList<RecyclingCenterCell> getRecyclingCenterList() { //Sean
        ArrayList<RecyclingCenterCell> RecyclingCells = new ArrayList<>();
        int N_ROWS = map.length;
        int N_COLS = map[0].length;
        for (int i=0;i<N_ROWS;i++){
            for(int j=0;j<N_COLS;j++){
                if(get(i, j) instanceof RecyclingCenterCell){
                    RecyclingCells.add((RecyclingCenterCell)get(i,j));
                }
            }
        }
        return RecyclingCells;
    }
}
